...
use serde_json::json; // 这里虽然叫 OpenAIRequest，其实是代理网关统一入口的格式
    ...
    async fn map_delta(
        ...
        if is_final {
            ...
            let message_delta = format!(
                r#"{{"type":"message_delta","delta":{{"stop_reason":"{}","stop_sequence":null}},"usage":{{"output_tokens":0}}}}"#,
                stop_reason
            );
            results.push(MapperChunk {
                ...
                if let Some(end_idx) = pending_text.find("</tool_call>") {
                    ...
                    if !trim_buf.is_empty() {
                        if let Ok(json_obj) = serde_json::from_str::<serde_json::Value>(trim_buf) {
                            ...
                            results.push(MapperChunk {
                                event: Some("content_block_stop".into()),
                                data: json!({ "type": "content_block_stop", "index": tool_idx })
                                    .to_string(),
                            });
                            *tool_call_index += 1;
                        } else {
