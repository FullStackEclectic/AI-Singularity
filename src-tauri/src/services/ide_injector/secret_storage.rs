...
fn decode_buffer_data(buffer: &serde_json::Value) -> Result<Vec<u8>, String> {
    ...
    for (idx, value) in data_arr.iter().enumerate() {
        let num = value
            .as_u64()
        ...
        encrypted_bytes.push(num as u8);
    }
...
fn load_existing_sessions(
    ...
    let encrypted_bytes = decode_buffer_data(&parsed)?;
    let prefix = detect_prefix(&encrypted_bytes).map(|value| value.to_string());
...
pub(super) fn inject_secret_to_state_db_for_codebuddy(
    db_path: &Path,
    db_key: &str,
    plaintext: &str,
) -> Result<(), String> {
...
pub(super) fn inject_secret_to_state_db_for_codebuddy_cn(
    db_path: &Path,
    db_key: &str,
    plaintext: &str,
) -> Result<(), String> {
...
pub(super) fn inject_secret_to_state_db_for_qoder(
    db_path: &Path,
    db_key: &str,
    plaintext: &str,
) -> Result<(), String> {
...
pub(super) fn inject_secret_to_state_db_for_workbuddy(
    db_path: &Path,
    db_key: &str,
    plaintext: &str,
) -> Result<(), String> {
        ...
        Ok(value) => {
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&value) {
                if let Ok(bytes) = decode_buffer_data(&parsed) {
                    detect_prefix(&bytes).map(|prefix| prefix.to_string())
                } else {
                    None
                }
            } else {
