...
impl IdeInjector {
    pub fn execute_injection(acc: &IdeAccount) -> crate::error::AppResult<()> {
        ...
        let result = if platform.contains("copilot") || platform.contains("cursor") {
            inject_copilot_token_for_user_data_dir("", &acc.email, &tk, None)
        } else {
            // Default fallback
            inject_copilot_token_for_user_data_dir("", &acc.email, &tk, None)
        };
        ...
        match result {
            Ok(_) => Ok(()),
            Err(e) => Err(crate::error::AppError::Other(anyhow::anyhow!("注入失败: {}", e))),
        }
    }
}
