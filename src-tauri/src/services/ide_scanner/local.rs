...
pub(super) fn collect_all_ide_state_db_candidates() -> Vec<(PathBuf, String)> {
...
pub(super) fn extract_gemini_local_account() -> Result<ScannedIdeAccount, String> {
...
pub(super) fn extract_codex_local_account() -> Result<ScannedIdeAccount, String> {
    let home = dirs::home_dir().ok_or("无法获取用户主目录".to_string())?;
...
pub(super) fn extract_cursor_local_account() -> Result<ScannedIdeAccount, String> {
...
pub(super) fn extract_windsurf_local_account() -> Result<ScannedIdeAccount, String> {
    ...
    let refresh_token =
        pick_string_recursive(&auth_status_json, &["refreshToken", "refresh_token"]);
...
pub(super) fn extract_qoder_local_account() -> Result<ScannedIdeAccount, String> {
    ...
    let user_info_json: serde_json::Value = serde_json::from_str(&user_info_raw)
        .map_err(|e| format!("解析 Qoder userInfo 失败: {}", e))?;
...
pub(super) fn extract_kiro_local_account() -> Result<ScannedIdeAccount, String> {
    ...
    let email = profile_json
        ...
        .or_else(|| pick_string_recursive(&auth_json, &["email", "upn", "preferred_username"]))
...
fn parse_codebuddy_secret(
    secret: &str,
    platform: &str,
    source_path: String,
) -> Result<ScannedIdeAccount, String> {
    ...
    let (uid_from_token, normalized_token) =
        if let Some((prefix, suffix)) = token_candidate.split_once('+') {
            let uid = prefix.trim();
            let token = suffix.trim();
            if token.is_empty() {
                return Err(format!(
                    "本地 {} 登录信息解析失败：access token 无效",
                    platform
                ));
            }
            (
...
pub(super) fn extract_codebuddy_cn_local_account() -> Result<ScannedIdeAccount, String> {
...
pub(super) fn extract_workbuddy_local_account() -> Result<ScannedIdeAccount, String> {
...
#[cfg(target_os = "macos")]
pub(super) fn extract_zed_local_account() -> Result<ScannedIdeAccount, String> {
    let meta_output =
...
pub(super) fn extract_trae_local_account() -> Result<ScannedIdeAccount, String> {
    ...
    let access_token = pick_string_recursive(&json, &["token", "accessToken", "access_token"]);
    let refresh_token = pick_string_recursive(&json, &["refreshToken", "refresh_token"]);
...
pub(super) fn get_v1_migration_paths() -> Vec<PathBuf> {
