...
pub const KNOWN_ENDPOINTS: &[(&str, &str, &str)] = &[
    ...
    ("deep_seek", "DeepSeek",           "https://api.deepseek.com/v1/models"),
    ("aliyun",    "阿里云百炼",          "https://dashscope.aliyuncs.com/compatible-mode/v1/models"),
    ("bytedance", "字节豆包",            "https://ark.cn-beijing.volces.com/api/v3/models"),
    ("moonshot",  "Moonshot (Kimi)",    "https://api.moonshot.cn/v1/models"),
    ("zhipu",     "智谱 GLM",            "https://open.bigmodel.cn/api/paas/v4/models"),
    ("nvidia_nim","NVIDIA NIM",         "https://integrate.api.nvidia.com/v1/models"),
];
...
impl SpeedTestService {
    ...
    pub async fn test_endpoint(platform: &str, display_name: &str, endpoint: &str) -> SpeedTestResult {
        ...
        let (latency_ms, status) = match result {
            Ok(resp) => {
                info!("SpeedTest {}: {}ms (HTTP {})", display_name, elapsed, resp.status());
