...
const TRAY_SCOPE_FILE: &str = "tray_scope.json";
const QUICK_SWITCH_MENU_PREFIX: &str = "quick_switch_account_";
const QUICK_SWITCH_MAX_PER_PLATFORM: usize = 8;

#[derive(Debug, Default, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct TrayScopeState {
    #[serde(default)]
...
fn save_tray_scope_state(app: &AppHandle, state: &TrayScopeState) -> Result<(), String> {
    ...
    let raw = serde_json::to_string_pretty(state).map_err(|e| format!("序列化托盘范围失败: {}", e))?;
...
fn is_attention_account(account: &IdeAccount) -> bool {
    account.is_proxy_disabled
        || account.disabled_reason.as_ref().is_some_and(|reason| !reason.trim().is_empty())
...
fn quick_switch_menu_id(platform: &str, account_id: &str) -> String {
    format!(
        "{}{}::{}",
...
fn parse_quick_switch_menu_id(id: &str) -> Option<(String, String)> {
...
fn format_accounts_summary_label(total: usize, current: usize, attention: usize) -> String {
    format!(
        "账号概览：总 {} · 当前 {} · 需关注 {}",
...
pub fn set_tray_platform_scope(app: &AppHandle, platforms: Vec<String>) -> Result<Vec<String>, String> {
...
fn build_menu(app: &AppHandle) -> Result<Menu<Wry>, tauri::Error> {
    ...
    let tray_scope = get_tray_platform_scope(app);
    ...
    let visible_providers = providers
        .into_iter()
        .filter(|provider| is_platform_in_scope(&tray_scope, &provider_platform_key(&provider.platform)))
        .collect::<Vec<_>>();
    ...
    let mut visible_accounts = db
        ...
        .unwrap_or_default()
        .into_iter()
        .filter(|account| is_platform_in_scope(&tray_scope, &account_platform_key(&account.origin_platform)))
        .collect::<Vec<_>>();

    let current_count = snapshots
        .iter()
        .filter(|item| item.account_id.as_ref().is_some_and(|value| !value.trim().is_empty()))
        .count();
    ...
    if !snapshots.is_empty() {
        ...
        for snapshot in snapshots.into_iter().filter(|item| item.account_id.is_some()) {
            let label = format!(
                "{}: {}",
                snapshot.platform,
                snapshot
...
mod tests {
    ...
    fn parse_quick_switch_id_works() {
        let id = quick_switch_menu_id("Codex", "acc-1");
        let parsed = parse_quick_switch_menu_id(&id);
        assert_eq!(
            parsed,
            Some(("codex".to_string(), "acc-1".to_string()))
        );
    }

    #[test]
    fn parse_quick_switch_id_rejects_invalid_payload() {
        assert_eq!(parse_quick_switch_menu_id("quick_switch_account_only_platform"), None);
        assert_eq!(parse_quick_switch_menu_id("something_else"), None);
    }

    #[test]
    fn account_summary_label_is_stable() {
        assert_eq!(
            format_accounts_summary_label(12, 4, 3),
