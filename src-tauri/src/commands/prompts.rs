...
#[tauri::command]
pub fn sync_prompt(db: State<'_, Database>, id: String, workspace_dir: String, app: AppHandle) -> AppResult<()> {
    let service = PromptService::new(&db);
...
#[tauri::command]
pub fn sync_prompt_to_tool(db: State<'_, Database>, id: String, app: AppHandle) -> AppResult<Vec<String>> {
    let service = PromptService::new(&db);
