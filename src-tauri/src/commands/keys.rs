        ...
        .map(
            |(
                ...
                ApiKey {
                    id,
                    name,
                    platform: serde_json::from_str::<Platform>(&format!("\"{}\"", platform_str))
                        .unwrap_or(Platform::Custom),
                    base_url,
                    key_preview,
                    status: serde_json::from_str::<KeyStatus>(&format!("\"{}\"", status_str))
...
pub async fn update_key(
    ...
    if let Some(priority) = request.priority {
        db.execute(
            "UPDATE api_keys SET priority = ?1 WHERE id = ?2",
            &[&priority, &request.id],
        )?;
    }
    Ok(())
