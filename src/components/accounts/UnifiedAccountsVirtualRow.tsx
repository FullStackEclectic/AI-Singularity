...
export function UnifiedAccountsVirtualRow({
  item,
  rowKey,
  rowIndex,
  rowSize,
  ...
  return (
    <div
      key={rowKey}
      className={`data-table-row ${compact ? "compact" : ""}`}
      data-index={rowIndex}
      style={{
        height: `${rowSize}px`,
    ...
    >
      ...
      <div className="col-time table-cell-ellipsis">
        {getItemTimeSummary(item)}
      </div>

      <div className="col-actions">
        <button className="btn-row-action" onClick={() => onCreateShareToken(item)} title="快速生成分享 Token">
          <Share size={14} />
        </button>
